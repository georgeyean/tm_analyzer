...
mainfile <- paste(tma_dir, "keyword_log.html", "/")


##
# Create html output
##
html_main_output<- function(model, sup, sub, sup_docs, sub_docs){
  ...
  for(z in 1:len(ords)){
    ...
    nums<- mean(sup.pis[,ords[z]])
    ee<- paste(z, keys[1], sep='. ')
    for(m in 2:10){
      ee<- paste(ee, keys[m], sep=',')
    }
    finalstr <- sprintf("%s<div class='sup_keys z-depth-1'><i class='adown'></i>  <span class='keys'>%s</span><div class='keys_books'>", finalstr, paste(ee, round(nums,3), sep=' | '))
    #find top ten documents for the super topic
    topbooks <- order(sup.pis[, ords[z]], decreasing=T)[0:10]
    for(t in 1:len(topbooks)){
      book_str = sprintf("<div class='sup_keys_book' val='%s'>%s | %s</div>", sup_docs[topbooks[t]],
                         sup_docs[topbooks[t]], round(sup.pis[topbooks[t],ords[z]],3))
      finalstr= sprintf("%s\n%s", finalstr, book_str)
    }
    finalstr <- sprintf("%s\n</div></div>", finalstr)
    ...
    for(h in 1:len(new_ords)){
      ...
      finalstr <- sprintf("%s<div class='sub_keys  z-depth-1'>\
                            ...
                            <div class='keys_books'>", finalstr, paste(ff, round(props[h],4), sep=' | '))
  ...
  write(finalstr, file=mainfile, append=TRUE)
}
...
html_top <- function(sup_range, sub_range){
    ...
    write("<html>",file=.GlobalEnv$mainfile, append=TRUE)
    ...
    js_css <- readChar("js_css_patch", file.info("js_css_patch")$size)
    js_css <- gsub("\n", "", js_css)
    htmlheader <- sprintf("<head><script src='%s'></script><link rel='stylesheet' href='%s'>\
            <link rel='stylesheet' href='%s'>%s</head><div>", jquerycdn, jqueycss, ggicon, js_css)
    write(htmlheader,file=.GlobalEnv$mainfile,append=TRUE)
    ...
    write(topics_header, file=mainfile, append=TRUE)
}
...
html_bottom <- function(){
    ...
    write("</div><html>",file=mainfile,append=TRUE)
    browseURL(mainfile)
}
